!(function ($) {
    ...
    $.fn.caldozaLayout = function (opts) {
        ...
        return this.each(function () {
            ...
            grid.addClass('_caldoza_grid_init');

            // setup loading
            grid.html('<div class="caldoza-loading" style=""></div>');
            ...
            var init_builder = function () {
                ...
                // remove loader
                grid.find('.caldoza-loading').remove();
            }

            var create_component = function (type, data) {

                var component = $('<div class="caldoza-component component-' + type + '" data-type="' + type + '" data-label="' + type + '"><div class="caldoza-component-toolbar"><span class="dashicons dashicons-admin-generic caldoza-component-edit"></span> ' + type + '<span class="dashicons dashicons-no remove-component right"></span></div></div>'),
            ...
            var remove_column = function (column) {
                ...
                var contents = column.find('.caldoza-component'),
                ...
                for (var r = 0; r < rows.length; r++) {
                    ...
                    for (var c = 0; c < columns.length; c++) {
                        ...
                        var column = $(columns[c]),
                            component_struct = [],
                            components = column.find('> .column-container > .caldoza-component');
            ...
            //  add element
            grid.on('click', '.caldoza-grid-element', function () {
                var clicked = $(this),
            ...
            grid.on('click', '.remove-component', function () {
                var component = $(this).closest('.caldoza-component');
                if (confirm('Really?')) {
            ...
            grid.on('click', '.remove-row', function () {
                var row = $(this).closest('.row'),
                    contents = row.find('.caldoza-component');
                if (contents.length) {
            ...
            grid.on('click', '.caldoza-component-edit', function () {
                var clicked = $(this),
                    component = clicked.closest('.caldoza-component'),
                    template_ele = $( '#' + grid.prop('id') + '-' + component.data('type') + '-tmpl' ),
                    template = Handlebars.compile( template_ele.html() );

                var component_modal = $.caldozaModal({
            ...
            grid.on('click', '.add-component', function () {
                ...
                current_modal = $.caldozaModal({
                    modal: 'caldoza_component',
                    title: 'Insert',
                    width : 550,
                    height: 445,
                    focus: true,
                ...
                $( current_modal.content ).on('click', '.caldoza-component-trigger', component_editor );
            ...
            function init_rows() {
                ...
                grid.sortable({
                    //handle : '.caldoza-row-toolbar',
                    //axis: "y",
                    connectWith: ".caldoza-control-layout",
                    delay: 150,
                    ...
                    helper: function (e, item) {
                        return $('<div class="caldoza-row-helper" style="width: 155px; height: 30px;"></div>');
                    },
                });

                sort_column_container = grid.find(".column-container").sortable({
                    //handle : '.caldoza-component-toolbar',
                    delay: 150,
                    forceHelperSize: false,
                    items: '.caldoza-component',
                    appendTo: "body",
                    connectWith: ".column-container",
                    cursorAt: {top: 35, left: 74},
                    zIndex: 1000000,
                    helper: function (e, item) {
                        return $('<div class="caldoza-component-helper" style="width: 155px; height: 30px;" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.data('label') + '</div>');
                    },
                    update: function () {
                ...
                grid.find('.row').each(function () {
                    ...
                    sort_column = row.find(".column").droppable({
                        accept: '.caldoza-caldozabuilder-layout-layout_grid-first_row-0-settings-conditions-mini',
                        greedy: true,
            ...
            /// components
            $(".caldoza-caldozabuilder-layout-layout_grid-first_row-0-settings-conditions-mini").draggable({
                cursor: "move",
                appendTo: "body",
                cursorAt: {top: 15},
                helper: function (event) {
                    var item = $(event.target);
                    return $('<div class="caldoza-component-helper" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.html() + '</div>');
                }
            });
    ...
    $(window).load(function () {
        $(document).on('caldoza.init', function () {
            $('.caldoza-control-layout').not('._caldoza_grid_init').caldozaLayout();
        });
    });
