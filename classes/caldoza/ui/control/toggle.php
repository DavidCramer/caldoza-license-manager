 ...
 * @since 1.0.0
 */
class toggle extends \caldoza\ui\control {
	...
	public function set_assets() {
		...
		$this->assets['style']['toggle'] = $this->url . 'assets/controls/toggle/css/toggle' . CLDZA_LMAN_ASSET_DEBUG . '.css';

		// Initilize core scripts
		$this->assets['script']['toggle-control-init'] = array(
			'src'       => $this->url . 'assets/controls/toggle/js/toggle' . CLDZA_LMAN_ASSET_DEBUG . '.js',
			'in_footer' => true,
		);
	...
	public function sdescription() {
		...
		if ( isset( $this->struct['description'] ) ) {
			$output .= '<span class="caldoza-toggle-description">' . esc_html( $this->struct['description'] ) . '</span>';
		}
	...
	protected function set_active_styles() {
		...
		if ( ! empty( $this->struct['base_color'] ) ) {
			$style .= '.' . $this->id() . ' > .caldoza-control-input > .switch.active {background: ' . $this->struct['base_color'] . ';}';
		}

		if ( ! empty( $this->struct['off_color'] ) ) {
			$style .= '.' . $this->id() . '> .caldoza-control-input > .switch { background: ' . $this->struct['off_color'] . ';}';
		}
