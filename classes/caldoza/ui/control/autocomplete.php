<?php
/**
 * CLDZA_LMAN Controls - Autocomplete
 *
 * @package   controls
 * @author    David Cramer
 * @license   GPL-2.0+
 ...
 * @since 1.0.0
 */
class autocomplete extends \caldoza\ui\control\select {
	...
	public function set_assets() {
		...
		$this->assets['style']['select2'] = $this->url . 'assets/controls/autocomplete/css/select2' . CLDZA_LMAN_ASSET_DEBUG . '.css';

		// Initilize core scripts
		$this->assets['script']['select2']      = array(
			'src'       => $this->url . 'assets/controls/autocomplete/js/select2' . CLDZA_LMAN_ASSET_DEBUG . '.js',
			'in_footer' => true,
		);
		$this->assets['script']['select2-init'] = array(
			'src'       => $this->url . 'assets/controls/autocomplete/js/select2-init' . CLDZA_LMAN_ASSET_DEBUG . '.js',
			'in_footer' => true,
		);
