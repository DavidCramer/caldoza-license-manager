 ...
 * @since 1.0.0
 */
class item extends \caldoza\ui\control {
	...
	public function setup() {
		...
		$this->struct['modal'] = array(
			'config' => array(
				'label'       => __( 'Add Item', 'caldoza' ),
				'description' => __( 'Setup Item', 'caldoza' ),
				'callback'    => array( $this, 'send_item_json' ),
				'attributes'  => array(
					'class'        => 'page-title-action',
					'data-content' => 'caldoza_item_control_modal',
				),
				'height'      => 540,
				'width'       => 380,
				'config'      => array(
					'target'  => 'caldoza_item_control_modal_handler',
					'control' => $this->id(),
				),
				'template'    => '{{json this}} <button type="button" class="caldoza-item-edit button button-small">' . esc_html__( 'Edit', 'caldoza' ) . '</button> | <button type="button" class="caldoza-item-remove button button-small">' . esc_html__( 'Remove', 'caldoza' ) . '</button>',
				'footer'      => array(
					'id'      => $this->slug . '_foot',
					'control' => array(
						'add_item'    => array(
							'label'      => __( 'Add', 'caldoza' ),
							'type'       => 'button',
							'attributes' => array(
								'type'       => 'submit',
								'data-state' => 'add',
		...
		add_action( 'caldoza_control_item_submit_' . $this->slug, $this->struct['modal']['config']['callback'], 100 );
	...
	public function handle_submit() {
		if ( isset( $this->child['config'] ) && $this->child['config']->is_submitted() ) {
			...
			do_action( 'caldoza_control_item_submit_' . $this->slug, $data, $this );
		}
	}
	...
	public function set_assets() {
		...
		$this->assets['style']['item']                 = $this->url . 'assets/css/item' . CLDZA_LMAN_ASSET_DEBUG . '.css';
		$this->assets['script']['handlebars']          = array(
			'src' => $this->url . 'assets/js/handlebars-latest' . CLDZA_LMAN_ASSET_DEBUG . '.js',
		);
		$this->assets['script']['baldrick-handlebars'] = array(
			'src'  => $this->url . 'assets/js/handlebars.baldrick' . CLDZA_LMAN_ASSET_DEBUG . '.js',
			'deps' => array( 'baldrick' ),
		);
		$this->assets['script']['item']                = array(
			'src' => $this->url . 'assets/js/item' . CLDZA_LMAN_ASSET_DEBUG . '.js',
		);
		parent::set_assets();
	}
	...
	public function render() {
		...
		$output .= '<div id="' . esc_attr( $this->id() ) . '" data-color="' . esc_attr( $this->base_color() ) . '" data-for="' . esc_attr( $this->id() ) . '-control" class="processing caldoza-control caldoza-control-' . esc_attr( $this->type ) . ' ' . esc_attr( $this->id() ) . '">';
		$output .= '</div>';
	...
	public function label() {
		...
		if ( isset( $this->struct['label'] ) ) {
			$output .= '<label for="' . esc_attr( $this->id() ) . '-control" class="caldoza-add-row"><span class="caldoza-control-label">' . esc_html( $this->struct['label'] ) . '</span></label>';
		}
	...
	protected function set_active_styles() {
		...
		$style .= '.' . $this->id() . ' .caldoza-component-toolbar{background-color: ' . $this->base_color() . ' !important;}';
