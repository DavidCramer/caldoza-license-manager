 ...
 * @since 1.0.0
 */
class layout extends \caldoza\ui\control {
	...
	public function set_assets() {
		...
		// set style
		$this->assets['style']['layout']      = $this->url . 'assets/controls/layout/css/layout' . CLDZA_LMAN_ASSET_DEBUG . '.css';
		$this->assets['style']['layout-grid'] = $this->url . 'assets/controls/layout/css/layout-grid' . CLDZA_LMAN_ASSET_DEBUG . '.css';

		// push to register script
		$this->assets['script']['layout']              = array(
			'src'       => $this->url . 'assets/controls/layout/js/layout' . CLDZA_LMAN_ASSET_DEBUG . '.js',
			'deps'      => array(
				'jquery-ui-draggable',
				'jquery-ui-droppable',
				'jquery-ui-sortable',
		...
		$this->assets['script']['modals'] = array(
			'src'  => $this->url . 'assets/js/modals' . CLDZA_LMAN_ASSET_DEBUG . '.js',
			'deps' => array( 'baldrick' ),
		);
		$this->assets['style']['modals']  = $this->url . 'assets/css/modals' . CLDZA_LMAN_ASSET_DEBUG . '.css';
	...
	public function render() {
		...
		$output = '<div id="' . esc_attr( $this->id() ) . '" data-color="' . esc_attr( $this->base_color() ) . '" data-for="' . esc_attr( $this->id() ) . '-control" class="caldoza-control caldoza-control-' . esc_attr( $this->type ) . ' ' . esc_attr( $this->id() ) . '"></div>';
		$output .= $this->label();
		$output .= $this->input();
	...
	public function component_templates() {
		...
		foreach ( $this->modals as $modal ) {
			...
			if ( isset( $modal->struct['preview'] ) ) {
				$template = caldoza()->add( 'control', $modal->slug, array(
					'type'     => 'template',
	...
	public function modal_template() {
		...
		foreach ( $this->modals as $modal ) {
			...
			$output .= '<button type="button" class="button caldoza-component-trigger" style="margin:12px 0 0 12px;" data-label="' . esc_attr( $modal->attributes['data-title'] ) . '" data-type="' . $modal->slug . '" ' . $setup . ' data-id="' . esc_attr( $modal->id() ) . '">' . $label . '</button> ';
