...
/**
 * CLDZA_LMAN Page class for creating admin/settings pages.
 * @package caldoza\ui
 * @author  David Cramer
 */
class page extends box implements \caldoza\data\save {
	...
	public function set_assets() {
		...
		$this->assets['style']['page'] = $this->url . 'assets/css/page' . CLDZA_LMAN_ASSET_DEBUG . '.css';
	...
	public function set_attributes() {
		...
		if ( empty( $this->struct['full_width'] ) ) {
			$this->attributes['class'] .= ' caldoza-half-page';
		}
	...
	/**
	 * get a CLDZA_LMAN config store key
	 * @since 1.0.0
	 * @access public
	 * @return string $store_key the defiuned option name for this CLDZA_LMAN object
	 */
	public function store_key() {
		...
		if ( ! empty( $this->struct['store_key'] ) ) {
			...
			$store_key = 'caldoza-' . $this->type . '-' . sanitize_text_field( $this->slug );
		}
	...
	protected function set_active_styles() {
		...
		$styles .= '.caldoza-page #panel-' . $this->id() . ' > .caldoza-panel-tabs > li[aria-selected="true"] a {box-shadow: 0 -3px 0 ' . $this->base_color() . ' inset;}';
		$styles .= '.caldoza-page #panel-' . $this->id() . '.caldoza-top-tabs > .caldoza-panel-tabs > li[aria-selected="true"] a {box-shadow: 0 3px 0 ' . $this->base_color() . ' inset;}';

		$styles .= '.contextual-help-tabs .active {border-left: 6px solid ' . $this->base_color() . ' !important;}';
		$styles .= '#' . $this->id() . '.caldoza-page h1{box-shadow: 0 0 2px rgba(0, 2, 0, 0.1),11px 0 0 ' . $this->base_color() . ' inset;}';
		$styles .= '#' . $this->id() . '.caldoza-page .page-title-action:hover{background: ' . $this->base_color() . ';border-color: rgba(0,0,0,0.1);}';
		$styles .= '#' . $this->id() . '.caldoza-page .page-title-action:focus{box-shadow: 0 0 2px ' . $this->base_color() . ';border-color: ' . $this->base_color() . ';}';

		if ( $this->child_count() > 1 ) {
			$styles .= '#' . $this->id() . '.caldoza-page h1{ box-shadow: 0 0px 13px 12px ' . $this->base_color() . ', 11px 0 0 ' . $this->base_color() . ' inset;}';
		}

		caldoza_share()->set_active_styles( $styles );
