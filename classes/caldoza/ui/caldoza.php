...
/**
 * Core CLDZA_LMAN abstract class.
 *
 * @package caldoza\ui
 * @author  David Cramer
 */
abstract class caldoza {
	 ...
	 * @param caldoza $parent Parent CLDZA_LMAN Object
	 */
	...
	public function setup() {
		...
		foreach ( $this->struct as $struct_key => $sub_struct ) {
			if ( is_array( $sub_struct ) && caldoza()->get_register_callback( $struct_key ) ) {
	...
	/**
	 * Define core CLDZA_LMAN styles - override to register core ( common styles for caldoza type )
	 *
	 * @since 1.0.0
	 * @access public
	 */
	...
	protected function actions() {
		...
		// init CLDZA_LMAN headers
		add_action( $location, array( $this, 'enqueue_core' ) );

	}

	/**
	 * Register the CLDZA_LMAN objects
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * @return object|\caldoza object instance
	 */
	 ...
	 *
	 * @return CLDZA_LMAN|null
	 */
	public function __call( $type, $args ) {
		$init  = caldoza()->get_register_callback( $type );
		$child = null;
		if ( null !== $init ) {
	...
	public function enqueue_core() {
		...
		// register caldoza core asset
		$this->core_assets();

		/**
		 * do object initilisation
		 *
		 * @param object current caldoza instance
		 */
		do_action( 'caldoza_admin_enqueue_scripts_' . $this->type, $this );
		 ...
		 * @param object current caldoza instance
		 */
		do_action( 'caldoza_admin_enqueue_scripts_' . $this->type . '_' . $this->slug, $this );

		// push assets to ui manager
		caldoza()->set_assets( $this->assets );
		// done enqueuing - dpo inline or manual enqueue.
		$this->set_active_styles();
	}

	/**
	 * Determin if a CLDZA_LMAN object should be active for this screen
	...
	/**
	 * Register CLDZA_LMAN depend js and css and call set assets
	 *
	 * @since 1.0.0
	 * @access protected
	 */
	protected function core_assets() {
		wp_register_script( 'caldoza', $this->url . 'assets/js/core' . CLDZA_LMAN_ASSET_DEBUG . '.js' );
		wp_register_style( 'caldoza', $this->url . 'assets/css/core' . CLDZA_LMAN_ASSET_DEBUG . '.css', array( 'dashicons' ) );
		wp_localize_script( 'caldoza', 'caldozaApi', array( 'root'  => esc_url_raw( rest_url() ),
	...
	protected function base_color() {
		 ...
		 * @param object current caldoza instance
		 */
		return apply_filters( 'caldoza_base_color_' . $this->type . '_' . $this->slug, $color );
