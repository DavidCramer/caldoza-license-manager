...
/**
 * Base CLDZA_LMAN Control class.
 *
 * @since       1.0.0
 */
class control extends \caldoza\data\data {
	...
	public static function register( $slug, $object, $parent = null ) {
		...
		if ( empty( $object['type'] ) || ! caldoza()->is_callable( 'control\\' . $object['type'] ) ) {
			$object['type'] = 'text';
		}
	...
	public function set_assets() {
		$this->assets['style']['controls'] = $this->url . 'assets/css/control' . CLDZA_LMAN_ASSET_DEBUG . '.css';
		parent::set_assets();
	}
	...
	public function render() {
		...
		$output = '<div id="' . esc_attr( $this->id() ) . '" class="caldoza-control caldoza-control-' . esc_attr( $this->type ) . ' ' . esc_attr( $this->id() ) . '">';

		$output .= $this->label();
		$output .= '<div class="caldoza-control-input">';
		$output .= $this->input();
		$output .= '</div>';
	...
	public function label() {
		...
		if ( isset( $this->struct['label'] ) ) {
			$output .= '<label for="' . esc_attr( $this->id() ) . '-control"><span class="caldoza-control-label">' . esc_html( $this->struct['label'] ) . '</span></label>';
		}
	...
	public function description() {
		...
		if ( isset( $this->struct['description'] ) ) {
			$output .= '<span class="caldoza-control-description">' . esc_html( $this->struct['description'] ) . '</span>';
		}
