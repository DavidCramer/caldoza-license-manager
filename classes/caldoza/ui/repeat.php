...
class repeat extends panel {
	...
	public function set_assets() {
		...
		$this->assets['script']['repeat'] = $this->url . 'assets/js/repeat' . CLDZA_LMAN_ASSET_DEBUG . '.js';
		$this->assets['style']['repeat']  = $this->url . 'assets/css/repeat' . CLDZA_LMAN_ASSET_DEBUG . '.css';
	...
	public function prepare_data() {
		$submit_data = caldoza()->request_vars( 'post' );
	...
	public function render() {
		...
		$output = '<div data-instance="{{json this}}" data-caldoza-template="' . esc_attr( $this->id() ) . '" ' . $this->build_attributes() . '>';
	...
	public function render_repeatable() {
		...
		$output = '<div class="caldoza-repeat">';
		$output .= $this->render_template();
		if ( ! empty( $this->child ) ) {
			$output .= $this->render_children();
		}

		$output .= '<button type="button" class="button button-small caldoza-remover"><span class="dashicons dashicons-no"></span></button> </div>';
	...
	public function render_repeatable_more() {
		...
		$this->instance  = 0;
		$output          = '<div class="repeatable-footer"><button type="button" class="button" data-caldoza-repeat="' . esc_attr( $this->id() ) . '">' . esc_html( $label ) . '</button></div>';
	...
	protected function set_active_styles() {
		...
		$style = '#' . $this->id() . ' .caldoza-repeat{ box-shadow: 1px 0 0 ' . $this->base_color() . ' inset, -37px 0 0 #f5f5f5 inset, -38px 0 0 #ddd inset, 0 2px 3px rgba(0, 0, 0, 0.05); };';
