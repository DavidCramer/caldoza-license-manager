 ...
 */
namespace caldoza\ui;

/**
 * CLDZA_LMAN panel. a holder to contain sections. a panel with multiple sections creates a tabbed interface to switch between sections areas.
 *
 * @package caldoza\ui
 * @author  David Cramer
 */
class panel extends \caldoza\data\data {
	...
	public function set_assets() {
		...
		$this->assets['script']['panel'] = $this->url . 'assets/js/panel' . CLDZA_LMAN_ASSET_DEBUG . '.js';
		$this->assets['style']['panel']  = $this->url . 'assets/css/panel' . CLDZA_LMAN_ASSET_DEBUG . '.css';
	...
	public function render() {
		...
		if ( $this->child_count() > 0 ) {
			...
			$output .= '<div id="panel-' . esc_attr( $this->id() ) . '" class="caldoza-' . esc_attr( $this->type ) . '-inside ' . esc_attr( $this->wrapper_class_names() ) . '">';
			// render a lable
			$output .= $this->label();
			// render a desciption
	 ...
	 * @access public
	 *
	 * @param caldoza Object to test if it is to be rendered in a section
	 *
	...
	public function wrapper_class_names() {
		...
		$wrapper_class_names = array(
			'caldoza-panel-inside',
		);

		if ( $this->child_count() > 1 ) {
			$wrapper_class_names[] = 'caldoza-has-tabs';

			if ( ! empty( $this->struct['top_tabs'] ) ) {
				$wrapper_class_names[] = 'caldoza-top-tabs';
			}
		}
	...
	public function label() {
		...
		if ( ! empty( $this->struct['label'] ) ) {
			$output .= '<div class="caldoza-' . esc_attr( $this->type ) . '-heading"><h3 class="caldoza-' . esc_attr( $this->type ) . '-title">' . esc_html( $this->struct['label'] ) . '</h3></div>';
		}
	...
	public function description() {
		...
		if ( ! empty( $this->struct['description'] ) ) {
			$output .= '<div class="caldoza-' . esc_attr( $this->type ) . '-heading"><p class="caldoza-' . esc_attr( $this->type ) . '-subtitle description">' . esc_html( $this->struct['description'] ) . '</p></div>';
		}
	...
	public function navigation() {
		...
		if ( $this->child_count() > 1 ) {
			...
			$output .= '<ul class="caldoza-' . esc_attr( $this->type ) . '-tabs caldoza-panel-tabs">';
			$active = 'true';
			foreach ( $this->child as $child ) {
				if ( $this->is_section_object( $child ) ) {
	...
	private function tab_label( $child, $active ) {
		...
		$output .= '<a href="#' . esc_attr( $child->id() ) . '" data-parent="' . esc_attr( $this->id() ) . '" class="caldoza-tab-trigger">' . $label . '</a>';
		$output .= '</li>';
	...
	public function panel_section() {
		...
		$output .= '<div class="caldoza-' . esc_attr( $this->type ) . '-sections caldoza-sections">';
	...
	protected function set_active_styles() {
		$style = '#panel-' . $this->id() . ' > .caldoza-panel-tabs > li[aria-selected="true"] a {box-shadow: 3px 0 0 ' . $this->base_color() . ' inset;}';
		$style .= '#panel-' . $this->id() . '.caldoza-top-tabs > .caldoza-panel-tabs > li[aria-selected="true"] a {	box-shadow: 0 3px 0 ' . $this->base_color() . ' inset;}';
