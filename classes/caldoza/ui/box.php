 ...
 * @author  David Cramer
 */
class box extends panel implements \caldoza\data\save, \caldoza\data\load {
	...
	public function init() {
		...
		if ( isset( $data[ 'caldozaNonce_' . $this->id() ] ) && wp_verify_nonce( $data[ 'caldozaNonce_' . $this->id() ], $this->id() ) ) {
			$this->save_data();
		} else {
			// load data normally
	...
	public function set_attributes() {
		...
		$action = caldoza()->request_vars( 'server' );
		$attributes = array(
			'enctype'  => 'multipart/form-data',
			'method'   => 'POST',
			'class'    => 'caldoza-ajax caldoza-' . $this->type,
			'data-caldoza' => $this->slug,
			'action'   => $action['REQUEST_URI'],
		);
		if ( ! empty( $this->struct['static'] ) ) {

			$attributes = array(
				'class'    => 'caldoza-' . $this->type,
				'data-caldoza' => $this->slug,
			);
		}
